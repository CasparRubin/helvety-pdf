...
export function generateRegistrationOptions(
  ...
  ;(options as PublicKeyCredentialCreationOptionsJSON & { hints?: string[] }).hints = ['hybrid']

  // Add PRF extension if salt provided
  if (prfSalt) {
    (options as PublicKeyCredentialCreationOptionsJSON & { extensions?: Record<string, unknown> }).extensions = {
      prf: {
        eval: {
...
export function generateAuthenticationOptions(
  ...
  ;(options as PublicKeyCredentialRequestOptionsJSON & { hints?: string[] }).hints = ['hybrid']

  // Add PRF extension if salt provided
  if (prfSalt) {
    (options as PublicKeyCredentialRequestOptionsJSON & { extensions?: Record<string, unknown> }).extensions = {
      prf: {
        eval: {
          first: base64Decode(prfSalt),
        },
