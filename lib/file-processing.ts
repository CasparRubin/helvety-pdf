...
function evictLRUEntry(
  ...
  if (cache.size >= maxSize) {
    ...
    if (firstKey) {
      ...
      logger.log(
        `LRU cache limit reached (${maxSize}), evicted least recently used: ${firstKey}`
      );
      return firstKey;
    }
  }
...
export async function processFile(
  ...
  if (!Number.isInteger(fileIndex) || fileIndex < 0) {
    return {
      error: `Invalid file index: ${fileIndex}. Must be a non-negative integer.`,
    };
  }
  ...
  try {
    ...
    const shouldYield =
      isMobile || isLargeFile || shouldYieldToBrowser(fileSize);
    ...
    const {
      ...
      fileType,
      inherentRotations,
    } = await loadFileWithPreview(file, isPdf);
    url = previewUrl;

    if (!url) {
      return {
        error: ERROR_TEMPLATES.ACTION_FAILED(
          "Create preview URL",
          `'${file.name}' could not create a preview URL`
        ),
      };
    }
    ...
    logger.log(
      `Cached PDF for ${fileType} file: "${file.name}" (ID: ${fileId}, cache size: ${pdfCache.size}/${cacheLimit})`
    );
