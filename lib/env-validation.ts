...
function validateAnonKey(key: string): boolean {
  ...
  const isNewFormat = trimmedKey.startsWith('sb_') || trimmedKey.startsWith('eyJ')
  const isJWTFormat = trimmedKey.includes('.') && trimmedKey.split('.').length >= 2
...
const envSchema = z.object({
  NEXT_PUBLIC_SUPABASE_PROJECT_URL: z
    ...
    .refine(
      ...
      {
        message: "NEXT_PUBLIC_SUPABASE_PROJECT_URL must start with http:// or https://",
      }
    ),
  NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY: z
    ...
    .refine(
      (key) => validateAnonKey(key),
      {
        message: "NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY must be a valid Supabase anon/publishable key. " +
...
export function getValidatedEnv(): Env {
  ...
  const rawEnv = {
    NEXT_PUBLIC_SUPABASE_PROJECT_URL: process.env.NEXT_PUBLIC_SUPABASE_PROJECT_URL?.trim() ?? "",
