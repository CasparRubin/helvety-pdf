 ...
 */
export function useImageBitmapMemory(
    ...
    const checkMemory = (): void => {
      try {
        ...
        if (systemMemoryHigh === true) {
          logger.warn(
            "System memory pressure detected, clearing ImageBitmap cache"
          );
          cache.clear();
          return;
        }
        ...
        const cacheMemoryHigh = isImageBitmapCacheMemoryHigh(
          stats,
          cacheThreshold
        );
